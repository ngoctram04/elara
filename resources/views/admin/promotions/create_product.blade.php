...
<form method="POST"
    ...
    <div class="card-body">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
            <h5 class="fw-semibold mb-0">Chọn sản phẩm / biến thể</h5>
        ...
        <div class="mb-3">
            ...
            <small class="text-muted">Gõ từ khóa để lọc nhanh danh sách sản phẩm và biến thể.</small>
        </div>
        ...
        <div id="productList">
            @foreach ($products as $product)
                @php
                    ...
                    $allLocked = count($allVariantIds) > 0 && $lockedCount === count($allVariantIds);

                    $oldSelected = collect(old("products.{$product->id}", []))->map(fn($id) => (int) $id)->toArray();
                    ...
                    <div class="variants-wrap ms-0 mt-3 {{ $shouldOpen ? '' : 'd-none' }}">
                        @foreach ($product->variants as $variant)
                            @php
                                ...
                                $isChecked = in_array($variant->id, old("products.{$product->id}", []));
                            @endphp
                                ...
                                <span class="flex-grow-1">
                                    ...
                                    @if($isLocked)
                                        <span class="small text-danger">
                                            <i class="bi bi-lock me-1"></i>Biến thể này đang có khuyến mãi
                                        </span>
                                    @endif
                                </span>
                            </label>
