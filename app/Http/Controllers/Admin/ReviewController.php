...
class ReviewController extends Controller
{
        ...
        if ($request->filled('keyword')) {
            ...
            $query->where(function ($q) use ($keyword, $numberKeyword) {
                $q->where('comment', 'like', '%' . $keyword . '%')
                    ->orWhereHas('orderItem.order.user', function ($q2) use ($keyword) {
                        $q2->where('name', 'like', '%' . $keyword . '%');
                    })
                    ->orWhereHas('orderItem.variant.product', function ($q3) use ($keyword) {
                        $q3->where('name', 'like', '%' . $keyword . '%');
                    })
                    ->orWhereRaw("CONCAT('DG', LPAD(id, 5, '0')) LIKE ?", ['%' . $keyword . '%'])
                    ->orWhereHas('orderItem.order', function ($q4) use ($keyword, $numberKeyword) {
                        $q4->whereRaw("CONCAT('DH', LPAD(id, 5, '0')) LIKE ?", ['%' . $keyword . '%']);

                        if ($numberKeyword !== '') {
                            $q4->orWhere('id', (int) $numberKeyword);
                        }
                    });
    ...
    public function reply(Request $request, $id)
    {
        ...
        $user = $review->orderItem?->order?->user;
        $productName = $review->orderItem?->variant?->product?->name ?? 'Sản phẩm';
